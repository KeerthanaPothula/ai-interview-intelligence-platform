...
| `pip-audit`/`npm-audit` (in `security.yml`) fails on a new advisory | A dependency (often a transitive one) has a newly published CVE that is not in the documented exceptions | These jobs are **blocking** — fix it (smallest version bump, re-run tests) or, if there is genuinely no compatible fix / the feature is unused, document it in `backend/pip-audit-exceptions.txt` / `frontend/audit-exceptions.json` per [SECURITY.md § Dependency Security](../SECURITY.md#dependency-security). Only the dev-tooling audit steps are non-blocking |
